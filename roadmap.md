...
| **Developer Ergonomics** | `mise`, `Makefile`, Docker Compose | `mise` owns toolchains, never dependencies: `mise.toml` pins runtimes and package managers (`go`, `node`, `pnpm`; `python` + `uv` when the Python bridge lands), the `gonext` CLI, and standalone dev binaries (`golangci-lint`, `gitleaks`, `lefthook`, `bru`); libraries and in-module Go tools (`wire`, `govulncheck`) stay in `go.mod` / `package.json`. The `gonext` CLI is mise-agnostic — it only needs `go` and `pnpm` on `PATH` — while the `Makefile`, `lefthook.yml` and CI wrap every command, `gonext` included, in `mise exec --` so the pinned toolchain applies where the shell hook never fires (agents, IDE-run git hooks, CI). Host prerequisites for a new project are therefore `mise` and Docker. |
...
| **Security Baseline** | `core/security`: response headers, in-process per-IP rate limiter | Every generated backend mounts `github.com/dennys-bd/gonext/core/security` from `api.NewEcho`: `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer` and a `default-src 'none'` CSP on every response (the CSP is skipped on Huma's `/docs`, which loads its reference UI from a CDN), plus HSTS wherever `config.IsRelaxedEnv` is false — the same rule that sets the session cookie's `Secure` attribute. A token-bucket limiter over Echo's memory store (`RATE_LIMIT_RPS`/`RATE_LIMIT_BURST`, `/healthz` exempt; `/readyz` counts, since it pings the database and probes arrive from their own address) answers `429` as `application/problem+json`, keyed by client IPv4 address or IPv6 /64; `TRUSTED_PROXIES` lists the CIDRs whose `X-Forwarded-For` is honoured, and an empty list means the TCP peer is the client. Per process by design — Feature Pack D's distributed limiter swaps the store through Echo's `RateLimiterStore`. No CORS: the frontend relays every call through Next's server, so the browser never reaches Go. |
...
| **Production Containerization** | Multi-stage `Dockerfile`s (backend + frontend) | Production-grade multi-stage Docker builds for backend and frontend, with `make` targets and CI updated to build/run via Docker instead of host toolchains. `mise` never enters an image: build stages use the official `golang:<ver>` / `node:<ver>` images (the version duplicated from `mise.toml` is accepted), and the runtime stage carries only the binary and real environment variables. Feeds directly into the Deployment Target feature pack (README.md), which selects the deploy target on top of these images. |
...
Dev-loop commands own their logic; `make`, lefthook and CI are callers of the CLI, never a second copy of the command.
...
Dev-loop is `gonext migrate`, `gonext dev`, `gonext generate migration <domain> <name>`, `gonext generate page <route> <operationId>`, `gonext generate resource <domain> <name> [field:type…]`, `gonext routes`, and `gonext generate`/`gonext generate --check` today — the last two run (or check) every codegen step a project has, wire today. `gonext generate migration <domain> <name>` writes the next-numbered migration skeleton into a domain. `gonext generate page <route> <operationId>` writes a Next.js route wired to one contract operation. `gonext generate resource <domain> <name> [field:type…]` writes a tested CRUD slice — entity, service, adapters, handlers, migration and Bruno requests — into an existing domain. `gonext routes` lists the contract's operations, their client call names, and the `frontend/app` routes that use them. `gonext migrate <domain>/<version>` brings one domain to one version in either direction (`<domain>/zero` meaning none of it applied), confirming a non-empty rollback unless `--yes`. The rest of the generator set — worker, doctor — is tracked on the board rather than inventoried here.
...
The first one, `auth/` (`github.com/dennys-bd/gonext/auth`), is built — see *Auth Provider Abstraction* in *Core Foundations* above. It is a package of the single gonext module rather than a separately versioned one: gonext publishes one version covering both the CLI and the library. `httpx` is the second — see *`httpx` is published* below, which is also the one place the sequencing rule is deliberately not followed. `dbmigrate` is the third, published for a different reason than the bar below: it is runtime code every generated project needs, and epic #37's runtime module is where such code is headed, so publishing it now beats extracting it from every scaffold later. It is the first published package with a third-party dependency (Bun). `core/security` is the fourth and the first under `core/`, the directory epic #37's runtime module is taking shape in.
