...
Dev-loop is `gonext migrate`, `gonext dev`, `gonext generate migration <domain> <name>`, `gonext generate page <route> <operationId>`, and `gonext generate`/`gonext generate --check` today — the last two run (or check) every codegen step a project has, wire today. `gonext generate migration <domain> <name>` writes the next-numbered migration skeleton into a domain. `gonext generate page <route> <operationId>` writes a Next.js route wired to one contract operation. `gonext migrate <domain>/<version>` brings one domain to one version in either direction (`<domain>/zero` meaning none of it applied), confirming a non-empty rollback unless `--yes`. The rest of the generator set — resource, worker, doctor — is tracked on the board rather than inventoried here.
