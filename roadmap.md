...
| **Auth Provider Abstraction** | Swappable provider interface (Clerk / Supabase / Auth0 adapters) | The provider contract is published as a stdlib-only nested module, `github.com/dennys-bd/gonext/auth` (`auth/`, tagged `auth/vX.Y.Z`), holding `Identity`, the `Resolver` port, the rule helpers and the context accessors. Generated projects import it rather than vendoring it, so a third-party adapter — Clerk, Supabase, Auth0 — can ship as an installable module compiled against a stable type; a scaffolded port could not be implemented from outside the project at all. Swapping provider is a two-line change to `users.ProvideSessionIssuer`/`users.ProvideResolver` in `backend/wire.go`. `gonext init` pins the contract version the CLI was built against. | Done |
...
The first one, `auth/` (`github.com/dennys-bd/gonext/auth`), is **built and published** — see *Auth Provider Abstraction* in *Core Foundations* above.
